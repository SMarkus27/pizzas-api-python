...
class CreateOrderService:
    ...
    async def create(self, data: CreateOrderInSchema):
        ...
        if store_quantity == 0:
            raise BadRequestException("Item is empty")

        if store_quantity < data.quantity:
            raise BadRequestException(f"Quantity must be greater than {store_quantity}")
        ...
        if not pizza_data:
            raise NotFoundException("Pizza not found")
