...
from src.services.orders.dependencies import get_create_order_service, get_all_order_service, get_order_service
...
async def create_order(data: CreateOrderInSchema, service: CreateOrderService = Depends(get_create_order_service)):
    data =  await service.create(data)
    return Response(data={"result": data}, status_code=status.HTTP_201_CREATED)

@router.get("/", status_code=status.HTTP_200_OK)
async def get_all_orders(page: int = Query(1), size: int = Query(10), service: GetAllOrderService = Depends(get_all_order_service)):
...
async def get_order(external_id: str, service: GetOrderService = Depends(get_order_service)):
